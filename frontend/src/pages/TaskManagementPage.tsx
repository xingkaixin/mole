...
import { Plus, Play, Search, Database as DatabaseIcon } from "lucide-react"
...
export function TaskManagementPage() {
	...
	const loadConnections = async () => {
		try {
			const { GetAllConnectionsWithMetadata } = await import("../../wailsjs/go/backend/App")
	...
	// 加载任务下的表
	const loadTaskTables = async (taskId: string) => {
		try {
	...
	const handleRemoveTable = async (tableId: string) => {
		...
		console.log("移除表:", { selectedTaskId, taskTableId: tableId, note: "tableId是tasks_tbls表的ID" })

		try {
			const { RemoveTableFromTask } = await import("../../wailsjs/go/backend/App")
		...
		<div className="p-8">
			...
			<div className="mb-8 flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground text-pretty">创建分析任务、添加表并执行分析</p>
				</div>
			...
			<div className="mb-6 flex items-center gap-4">
						...
						<SelectContent>
							{tasks.map((task) => (
								<SelectItem key={task.id} value={task.id}>
									{task.name} ({task.tables?.length || 0} 个表)
								</SelectItem>
							))}
						</SelectContent>
					</Select>
				...
				{selectedTask && (
					<div className="flex gap-2">
						<Button onClick={() => setAddTableDialogOpen(true)} variant="outline">
							<Plus className="w-4 h-4 mr-2" />
							添加表
						</Button>
						<Button onClick={handleStartAnalysis} disabled={!selectedTask.tables || selectedTask.tables.length === 0}>
							...
							{filteredTables.length > 0 ? (
								filteredTables.map((table) => (
									<TableRow key={table.id}>
										...
										<TableCell className="text-right">{formatSize(table.tableSize)}</TableCell>
