...
export function TaskManagementPage({ onNavigateToAnalysisDetail }: TaskManagementPageProps) {
  ...
  // 加载任务下的表
  const loadTaskTables = useCallback(
    async (taskId: string) => {
      try {
                        ...
                        {table.tblStatus === "分析中" ? (
                          ...
                          >
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleViewAnalysisDetail(table.tableId)}
                              title="查看分析详情"
                            >
