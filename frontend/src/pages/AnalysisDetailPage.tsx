...
import { GetEnhancedAnalysisResult } from "../../wailsjs/go/backend/App";
...
export function AnalysisDetailPage({ result, onBack }: AnalysisDetailPageProps) {
	...
	// 获取行数
	const rowCount = enhancedResult?.results?.row_count || analysisData?.row_count || 0;
	...
	if (enhancedResult?.columns) {
		...
		enhancedResult.columns.forEach((column) => {
			columns.push({
				...
				nonNullRate: nonNullRateData[column.name] ? Math.round(nonNullRateData[column.name] * 100) : undefined,
			});
		});
	} else if (analysisData?.non_null_rate) {
		// 降级使用原有数据结构
		Object.entries(analysisData.non_null_rate).forEach(([name, nonNullRate]) => {
			columns.push({
				name,
				type: "未知",
				comment: "",
	...
	// 按字段序号排序
	const sortedColumns = [...filteredColumns].sort((a, b) => a.ordinal - b.ordinal);
						...
						<Table>
							<TableHeader className="sticky top-0 bg-white z-10">
								<TableRow>
									...
									{sortedColumns.some(col => col.nonNullRate !== undefined) && (
